import { MeetingFormRefType } from "@/types/Meeting";
import { forwardRef } from "react";
import Button from "../Common/Button";
import Input from "../Common/Input";

interface MeetingInfoInputProps {
  navigateStep: (stepOffset: number) => void;
}

const MeetingInfoInput = forwardRef<MeetingFormRefType, MeetingInfoInputProps>(
  function MeetingInfoInput({ navigateStep }: MeetingInfoInputProps, ref) {
    return (
      <>
        <h3>어떤 약속인가요?</h3>
        <Input
          id="title"
          label="약속명을 입력해주세요."
          type="text"
          placeholder="해커톤 뒷풀이"
          required
          ref={ref}
        />

        <h3>몇명이서 모이나요?</h3>
        <Input
          id="memberCnt"
          label="모임 인원 수를 입력해주세요."
          type="number"
          placeholder="5"
          required
          min={1}
          max={40}
          ref={ref}
        />

        <Button onClick={() => navigateStep(1)}>다음</Button>
      </>
    );
  }
);

export default MeetingInfoInput;
